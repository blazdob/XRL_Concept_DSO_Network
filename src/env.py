...
class VoltageControlEnv(gym.Env):
	...
	def step(self, action):
		...
		# apply current action to the network
		current_load = self.load.curr() * 10

		self.current_net.load.p_mw = current_load
		try:
			pp.runpp(self.current_net)
			new_unchanged_state = []
			if "vm_pu" in self.state_types:
				new_unchanged_state += list(self.current_net.res_bus.vm_pu.values.astype(np.float32))
			if "load_p_mw" in self.state_types:
				new_unchanged_state += list(self.current_net.load.p_mw.values.astype(np.float32))
			self.new_unchanged_state = np.array(new_unchanged_state)
		except pp.LoadflowNotConverged:
			print("error in runpp, with the unchanged state")
		
		if self.new_unchanged_state[len(self.current_net.bus):] < self.v_upper:
			action = np.zeros(len(action))


		############# TAKING THE ACTION ################
		solvable, new_state = self._take_action2(action)
	...
	def _calc_reward(self, new_state, action, info={}):
		...
		new_states_where_old_exceeded = np.where(self.new_unchanged_state[:len(v)] > self.v_upper, new_state[:len(v)], 0)
		
		# new_states_where_old_not_exceed = np.where(self.new_unchanged_state[:len(v)] < self.v_upper, new_state[:len(v)], 0)
		...
		for el in new_states_where_old_exceeded:
			if el != 0:
				intermediate_reward -= max(0, (el - self.v_upper))**2*20000
		
		if self.new_unchanged_state.max() < self.v_upper:
			intermediate_reward -= np.sum(action)/100
	...
	def _take_action2(self, action):
		...
		for i, active_i in enumerate(self.active_consumers):
			if current_load[active_i] <= 0.0:
				if action[i] > 0.0:
					loads[active_i] = current_load[active_i] * (1 - action[i])
		
		self.current_net.load.p_mw = loads
		# take a step
		self.load.next()
		...
		try:
			...
			state = []
			if "vm_pu" in self.state_types:
				state += list(self.current_net.res_bus.vm_pu.values.astype(np.float32))
			if "load_p_mw" in self.state_types:
				state += list(self.current_net.load.p_mw.values.astype(np.float32))
			self.state = np.array(state)
			return True, self.state
		except pp.LoadflowNotConverged:
			#if loadflow could not be calculated the reward should be negative, 
            #since the state is not valid
	...
	def _take_action(self, action):
		...
		next_load = self.load.next() * 10

		# calculate what the conditions would be if we did not do the action
		self.current_net.load.p_mw = next_load
		try:
			pp.runpp(self.current_net)
			self.new_unchanged_state = self.current_net.res_bus.vm_pu.values.astype(np.float32)
		except:
			print("error in runpp, with the unchanged state")
			return False, self.state
		...
		next_updated_loads = next_load.copy()

		# fill the vectors with the next values
		for i, active_i in enumerate(self.active_consumers):
			if next_load[active_i] <= 0.0:
				next_updated_loads[active_i] = next_load[active_i] * (1 - action[i])

		self.current_net.load.p_mw = next_updated_loads #(next_updated_loads * (next_updated_loads < 0.0)) + (next_updated_loads * (next_updated_loads >= 0.0))
		try:
			pp.runpp(self.current_net)
	...
	def reset(self, starting=None):
		...
		while not solvable:
			...
			if starting != None:
				...
				# self.generation.set_random_starting_pos(self.episode_limit)
			...
			try:
				...
				solvable = False
		# convert to float32 to make it more general (in case we want to use continuous actions)
		observation = self.current_net.res_bus.vm_pu.values.astype(np.float32)
		return observation

	def plot_scenario_explanation(self, obs, action, rewards):
		"""
		Function plots the different possible scenarios for the current state
		"""
